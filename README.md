# Analysis of Non-Markovian Stochastic Petri Nets (NMSPNs)

This repository hosts the implementation of **Example 3 (pages 33-38)**. The code provides a framework for analyzing NMSPN models, with a fixed network structure but adjustable parameters.

## Requirements

The code is written in Python 3.9 and relies on the following standard libraries:

- `copy` — for deep copying objects
- `numpy` — for numerical computations
- `scipy` — for scientific algorithms
- `datetime` — for handling date/time operations

## Code Overview

- This repository only contains one function, and the execution result of this function is **the cumulative firing counts of transitions**.
- Comments have been provided in the code for some key variables and equations.
